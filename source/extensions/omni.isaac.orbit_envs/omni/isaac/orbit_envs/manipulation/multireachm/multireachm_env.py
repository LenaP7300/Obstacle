...
from .multireachm_cfg import RandomizationCfg, ReachEnvCfg


class ReachEnv(IsaacEnv):
    """Environment for reaching to desired pose for a single-arm manipulator."""

    def __init__(self, only_image: bool = False, res: int = 128, image_type: str = "rgb", logdir: str = None, video: bool = False, cfg: ReachEnvCfg = None, headless: bool = False):
        ...
        self.cylinder = RigidObject(cfg=self.cfg.cylinder)
        ...
        self.observation_manager = ReachObservationManager(class_to_dict(self.cfg.observations), self, self.device)
        ...
        num_obs = self.observation_manager._group_obs_dim["policy"][0]

        if self.only_image:
            self.observation_space = gym.spaces.Dict({"image": gym.spaces.Box(low=0, high=255, shape=(self.channels, self.res, self.res), dtype=np.uint8),
                                                      "vector": gym.spaces.Box(low=-math.inf, high=math.inf, shape=(num_obs,))})
        else:
            self.observation_space = gym.spaces.Dict({"image": gym.spaces.Box(low=0, high=255, shape=(self.channels, self.res, self.res), dtype=np.uint8)})
        ...
        self.cylinder.update_buffers(self.dt)
    ...
    def _design_scene(self):
        ...
        # cylinder
        self.cylinder.spawn(self.template_env_ns + "/Cylinder")
        # robot
    ...
    def _reset_idx(self, env_ids: VecEnvIndices):
        ...
        root_state = self.cylinder.get_default_root_state(env_ids)
        # transform command from local env to world
        root_state[:, 0:3] += self.envs_positions[env_ids]
        # set the root state
        self.cylinder.set_root_state(root_state, env_ids=env_ids)
    ...
    def _get_observations(self) -> VecEnvObs:
        ...
        Vectors = self.observation_manager.compute()["policy"].tolist()

        Images = self.env_image()
        image_tensor = torch.tensor(Images, device=self.device, dtype=torch.float32)

        if self.only_image:
            vector_tensor = torch.tensor(Vectors, device=self.device, dtype=torch.float32)
            return {'policy': {'vector': vector_tensor, 'image': image_tensor}}
        else:
    ...
    def _initialize_views(self) -> None:
        ...
        self.cylinder.initialize(self.env_ns + "/.*/Cylinder")
    ...
    def _check_termination(self) -> None:
        ee_distance = torch.norm(self.robot.data.ee_state_w[:, 0:3] - self.cylinder.data.root_pos_w, dim=1)
        # extract values from buffer
        # compute resets
        self.reset_buf[:] = 0
        # -- episode length
    ...
    def env_image(self):
        ...
        for rp in self.rp_list:
            ...
            if len(image) != 0:
                ...
                image_shaped = image.reshape((self.channels, self.res, self.res))
        ...
        if len(Images) == 0:
            Images = np.zeros((self.num_envs, self.channels, self.res, self.res), dtype=np.uint8)
...
class ReachObservationManager(ObservationManager):
    ...
    def object_position(self, env: ReachEnv):
        return env.cylinder.data.root_pos_w - env.envs_positions
...
class ReachRewardManager(RewardManager):
    ...
    def hitting_object_l2(self, env: ReachEnv):
        ee_distance = torch.norm(env.robot.data.ee_state_w[:, 0:3] - env.cylinder.data.root_pos_w, dim=1).cpu().detach().numpy()
        collision = np.zeros((env.num_envs,))
        for env_num in range(0, len(ee_distance)):
            if ee_distance[env_num] < 0.1:
                if ee_distance[env_num] != 0.0:
                    collision[env_num] = 1 / ee_distance[env_num]
                else:
                    collision[env_num] = 1000
        """Penalize end-effector tracking position error using L2-kernel."""
        return torch.sum(torch.square(torch.tensor(collision)))
