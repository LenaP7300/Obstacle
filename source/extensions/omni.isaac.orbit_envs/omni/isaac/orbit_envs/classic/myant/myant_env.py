...
class AntEnv(IsaacEnv):
    ...
    def __init__(self, cfg: dict, only_image: bool = False, res: int = 0, image_type: str = "rgb", logdir: str = None, video: bool = False, headless: bool = False):
        ...
        if not self.only_image:
            self.observation_space = gym.spaces.Dict({"image": gym.spaces.Box(low=0, high=255, shape=(self.res * self.res,), dtype=np.float32),
                                                      "vector": gym.spaces.Box(low=-math.inf, high=math.inf, shape=(60,))})
        else:
            self.observation_space = gym.spaces.Dict({"image": gym.spaces.Box(low=0, high=255, shape=(self.res * self.res,), dtype=np.float32)})
    ...
    def _design_scene(self) -> List[str]:
        ...
        set_camera_view([0.0, 0.0, 7.0], [1.0 * math.pi, 0.0, 0.75 * math.pi], self.camera.prim_path)
        for env_num in range(0, self.num_envs):
            rp = rep.create.render_product("/World/envs/env_" + str(env_num) + "/Camera/Camera", resolution=(self.res, self.res))
    ...
    def _get_observations(self) -> VecEnvObs:
        ...
        if not self.only_image:
            ...
            return {'policy': {'vector': obs_buf, 'image': image_tensor}}
        else:
