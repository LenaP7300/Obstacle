...
class CartpoleEnv(IsaacEnv):
    ...
    def __init__(self, cfg: dict, res: int = 128, image_type: str = "rgb", logdir: str = None, video: bool = False, headless: bool = False):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(self.res, self.height, self.channels), dtype=np.uint8)
    ...
    def _get_observations(self) -> VecEnvObs:
        ...
        for rp in self.rp_list:
            ...
            if len(image) != 0:
                ...
                image_shaped = image.reshape(self.res, self.height, self.channels)
                image_bits = np.asarray(image_shaped).astype(np.uint8)
                Images.append(image_bits)
                if self.video and self.step_count % (500 * self.num_envs) == 0:
                    filename = self.logdir + "/images/image_" + str(self.step_count) + ".png"
                    cv2.imwrite(filename, image)
        if len(Images) == 0:
            print("It is not getting any data.")
            Images = np.zeros((self.num_envs, self.res, self.height, self.channels), dtype=np.uint8)
        return {"policy": torch.tensor(np.asarray(Images), device=self.device, dtype=torch.uint8)}
